...
public class SpaceResourceIT {
    ...
    @Test
    @Transactional
    public void getAllSpacesByUserInfoIsEqualToSomething() throws Exception {
        // Initialize the database
        spaceRepository.saveAndFlush(space);
        UserInfo userInfo = UserInfoResourceIT.createEntity(em);
        em.persist(userInfo);
        em.flush();
        space.setUserInfo(userInfo);
        spaceRepository.saveAndFlush(space);
        Long userInfoId = userInfo.getId();

        // Get all the spaceList where userInfo equals to userInfoId
        defaultSpaceShouldBeFound("userInfoId.equals=" + userInfoId);

        // Get all the spaceList where userInfo equals to userInfoId + 1
        defaultSpaceShouldNotBeFound("userInfoId.equals=" + (userInfoId + 1));
    }
